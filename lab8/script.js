...
function init() {
    ...
    saveWork = !saveWork;
    //if (!saveWork) mypoints=[vec2(  0.00 ,  0.00 )];
    mypoints=[vec2(  0.00 ,  0.00 )];
    render();
    });
...
function render() {
    gl.bufferSubData(gl.ARRAY_BUFFER , 0 , flatten(mypoints));
    gl.clear( gl.COLOR_BUFFER_BIT );
    gl.drawArrays( gl.POINTS , 1 , mypoints.length-1 );
}
